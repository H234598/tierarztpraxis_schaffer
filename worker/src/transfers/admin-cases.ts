...
import { generateTransferToken, generateTransferTokenForPublicCaseId } from "./tokens";
import { createAdminReply, ReplyInputError, validateAdminReplyInput } from "./replies";
import { enqueueNotification, findPendingNotificationForReply } from "./notifications";
...
function mapSubmission(row: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    callbackPhone: row.callback_phone,
...
async function createCase(
  ...
  await database.batch([
    database
      ...
      .bind(
        caseId,
        generated.storage.publicCaseId,
        input.petName,
        input.ownerDisplayName,
        input.internalReference,
        input.internalNote,
        input.allowReplies ? 1 : 0,
        input.allowCallback ? 1 : 0,
  ...
  return json(
    {
      ...
      case: {
        id: caseId,
        publicId: generated.storage.publicCaseId,
        ...
        allowCallback: input.allowCallback,
...
function listWhere(url: URL): { clause: string; values: unknown[]; page: number } {
  ...
  if (q) {
    predicates.push(
      "(pet_name LIKE ? ESCAPE '\\' OR public_id LIKE ? ESCAPE '\\' OR owner_display_name LIKE ? ESCAPE '\\' OR internal_reference LIKE ? ESCAPE '\\')",
    );
...
async function listCases(context: DevelopmentRouteContext): Promise<Response> {
  ...
  const count = await database
    .prepare(`SELECT COUNT(*) AS total FROM transfer_cases ${clause}`)
    .bind(...values)
    .first<{ total: number }>();
  const rows = await database
    ...
    .bind(...values, PAGE_SIZE, (page - 1) * PAGE_SIZE)
    .all<Record<string, unknown>>();
  return json({
    ok: true,
    cases: rows.results.map(adminCaseSummary),
    page,
    pageSize: PAGE_SIZE,
    total: count?.total ?? 0,
  });
...
async function caseDetail(
  ...
  const [submissions, files, links, replies, tokens, audit] = await Promise.all([
    database
      .prepare(
        `SELECT id, title, message, observed_since, urgency, callback_requested, callback_phone, notification_email, status, created_at, finalized_at, updated_at FROM transfer_submissions WHERE case_id = ? ORDER BY created_at ASC`,
      )
      .bind(caseId)
      .all<Record<string, unknown>>(),
    database
      .prepare(
        `SELECT id, submission_id, original_name, declared_media_type, verified_media_type, expected_size, stored_size, state, inline_safe, created_at, uploaded_at FROM transfer_files WHERE case_id = ? ORDER BY created_at ASC`,
      )
      .bind(caseId)
      .all<Record<string, unknown>>(),
    database
      .prepare(
        `SELECT l.id, l.submission_id, l.url, l.label, l.created_at FROM transfer_links AS l INNER JOIN transfer_submissions AS s ON s.id = l.submission_id WHERE s.case_id = ? ORDER BY l.created_at ASC`,
      )
      .bind(caseId)
      .all<Record<string, unknown>>(),
    database
      .prepare(
        `SELECT id, submission_id, body, created_by_email, created_at FROM transfer_replies WHERE case_id = ? ORDER BY created_at ASC`,
      )
      .bind(caseId)
      .all<Record<string, unknown>>(),
    database
      .prepare(
        `SELECT id, token_version, token_hint, created_at, expires_at, revoked_at, last_used_at, use_count FROM transfer_tokens WHERE case_id = ? ORDER BY created_at DESC`,
      )
      .bind(caseId)
      .all<Record<string, unknown>>(),
    database
      .prepare(
        `SELECT id, event_type, actor_type, actor_reference, details_json, created_at FROM transfer_audit_events WHERE case_id = ? ORDER BY created_at DESC, id DESC LIMIT 50`,
      )
      .bind(caseId)
      .all<Record<string, unknown>>(),
  ]);
...
async function createToken(
  ...
  const body = validateExactBody(await readJson(context), [
    "revokeExisting",
    "expiresInDays",
  ]);
  ...
  const database = context.env.TRANSFER_DB;
  const transferCase = await database
    .prepare(
      "SELECT id, public_id, status, expires_at FROM transfer_cases WHERE id = ? LIMIT 1",
    )
    .bind(caseId)
    .first<{ id: string; public_id: string; status: string; expires_at: string }>();
  if (!transferCase)
    return transferError(context.requestId, 404, "not_found", "Not found");
  if (
    transferCase.status !== "open" ||
    Date.parse(transferCase.expires_at) <= Date.now()
  )
    return transferError(context.requestId, 409, "invalid_state", "Invalid state");
  const generated = await generateTransferTokenForPublicCaseId(
    transferCase.public_id,
    context.env.TOKEN_PEPPER,
  );
  const tokenId = crypto.randomUUID();
  const nowDate = new Date();
  const now = nowDate.toISOString();
  const expiresAt = new Date(
    Math.min(
      Date.parse(transferCase.expires_at),
      Date.parse(isoAfter(nowDate, expiresInDays)),
    ),
  ).toISOString();
  const statements: D1PreparedStatement[] = [
    database
      .prepare(
        `INSERT INTO transfer_tokens (id, case_id, token_version, token_hmac, token_hint, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      )
      .bind(
        tokenId,
        caseId,
  ...
  if (revokeExisting) {
    statements.push(
      database
        .prepare(
          `UPDATE transfer_tokens SET revoked_at = ? WHERE case_id = ? AND id <> ? AND revoked_at IS NULL AND EXISTS (SELECT 1 FROM transfer_tokens WHERE id = ? AND case_id = ?)`,
        )
        .bind(now, caseId, tokenId, tokenId, caseId),
      database
        .prepare(
          `UPDATE transfer_sessions SET revoked_at = ? WHERE case_id = ? AND revoked_at IS NULL AND EXISTS (SELECT 1 FROM transfer_tokens WHERE id = ? AND case_id = ?)`,
        )
        .bind(now, caseId, tokenId, caseId),
    );
  }
  statements.push(
    auditStatement(
...
async function revokeToken(
  ...
  await database.batch([
    ...
    database
      .prepare(
        "UPDATE transfer_sessions SET revoked_at = COALESCE(revoked_at, ?) WHERE token_id = ?",
      )
      .bind(now, tokenId),
    database
      .prepare(
        `INSERT INTO transfer_audit_events (id, case_id, event_type, actor_type, actor_reference, details_json, created_at) SELECT ?, case_id, 'token_revoked', 'admin', ?, ?, ? FROM transfer_tokens WHERE id = ? AND revoked_at = ?`,
      )
      .bind(crypto.randomUUID(), admin.subject, auditDetails(admin), now, tokenId, now),
  ]);
  return json({ ok: true });
}
...
async function updateStatus(
  ...
  if (body.status !== "open" && body.status !== "closed")
    throw new AdminInputError("status");
  const database = context.env.TRANSFER_DB;
  ...
  const results = await database.batch([
    database
      .prepare(
        `UPDATE transfer_cases SET status = ?, updated_at = ?, closed_at = ? WHERE id = ? AND status = '${from}'`,
      )
      .bind(body.status, now, body.status === "closed" ? now : null, caseId),
    database
      .prepare(
        `INSERT INTO transfer_audit_events
      (id, case_id, event_type, actor_type, actor_reference, details_json, created_at)
      ...
      .bind(
        crypto.randomUUID(),
        body.status === "closed" ? "case_closed" : "case_reopened",
        admin.subject,
        auditDetails(admin),
        now,
        caseId,
...
async function markExported(
  ...
  const database = context.env.TRANSFER_DB;
  const current = await database
    .prepare("SELECT exported_at FROM transfer_cases WHERE id = ? LIMIT 1")
    .bind(caseId)
  ...
  const results = await database.batch([
    database
      .prepare(
        "UPDATE transfer_cases SET exported_at = ?, updated_at = ? WHERE id = ? AND exported_at IS NULL",
      )
      .bind(now, now, caseId),
    database
      .prepare(
      ...
      WHERE id = ? AND exported_at = ?`,
      )
      .bind(crypto.randomUUID(), admin.subject, auditDetails(admin), now, caseId, now),
  ]);
  if ((results[0]?.meta.changes ?? 0) !== 1)
    return transferError(context.requestId, 409, "invalid_state", "Invalid state");
...
async function createReply(
  ...
  const input = validateAdminReplyInput(await readJson(context));
  ...
  if (result.notificationRequested && result.replyId) {
    try {
      const notificationId = await findPendingNotificationForReply(
        context.env.TRANSFER_DB,
        result.replyId,
      );
      if (notificationId) {
...
export async function handleAdminCaseApi(
  context: DevelopmentRouteContext,
  admin: VerifiedAdminIdentity,
): Promise<Response | null> {
  const { pathname } = context.url;
  try {
    if (pathname === "/api/admin/cases" && context.request.method === "POST")
      return await createCase(context, admin);
    if (pathname === "/api/admin/cases" && context.request.method === "GET")
      return await listCases(context);
    const tokenRevoke = pathname.match(/^\/api\/admin\/tokens\/([^/]+)\/revoke$/u)?.[1];
    if (tokenRevoke && idPattern.test(tokenRevoke) && context.request.method === "POST")
      return await revokeToken(context, admin, tokenRevoke);
    const caseAction = pathname.match(
      /^\/api\/admin\/cases\/([^/]+)\/(tokens|status|mark-exported|replies)$/u,
    );
    if (caseAction?.[1] && idPattern.test(caseAction[1])) {
      if (caseAction[2] === "tokens" && context.request.method === "POST")
        return await createToken(context, admin, caseAction[1]);
      if (caseAction[2] === "status" && context.request.method === "PATCH")
        return await updateStatus(context, admin, caseAction[1]);
      if (caseAction[2] === "mark-exported" && context.request.method === "POST")
        return await markExported(context, admin, caseAction[1]);
      if (caseAction[2] === "replies" && context.request.method === "POST")
        return await createReply(context, admin, caseAction[1]);
    }
    const detailId = pathname.match(/^\/api\/admin\/cases\/([^/]+)$/u)?.[1];
    if (detailId && idPattern.test(detailId) && context.request.method === "GET")
      return await caseDetail(context, detailId);
    return null;
  } catch (error) {
    if (error instanceof AdminInputError || error instanceof ReplyInputError) {
      return transferError(
        context.requestId,
        error.field === "contentType" ? 415 : 400,
        "invalid_request",
        "Invalid request",
        [error.field],
      );
