...
export function parseSingleRange(
  ...
  if (!Number.isSafeInteger(size) || size <= 0 || !/^bytes=\d*-\d*$/u.test(header))
    return "invalid";
  ...
  for (let index = 0; index < name.length; index += 1) {
    ...
    if (unit >= 0xd800 && unit <= 0xdbff && index + 1 < name.length) {
      ...
      if (next >= 0xdc00 && next <= 0xdfff) {
        wellFormed += name[index]! + name[index + 1]!;
        index += 1;
        continue;
      }
    }
...
export async function storedFileResponse(
  ...
  if (
    ...
    (metadata.inlineSafe !== 0 && metadata.inlineSafe !== 1) ||
    !isAllowedMediaType(metadata.mediaType)
  ) {
    return "unavailable";
  }
  const inline =
    metadata.inlineSafe === 1 && inlineSafeMediaTypes.has(metadata.mediaType);
  const rangeCapable =
    inline &&
    (metadata.mediaType === "video/mp4" || metadata.mediaType === "video/webm");
  ...
  try {
    ...
    headers.set(
      "content-disposition",
      contentDisposition(metadata.originalName, inline),
    );
