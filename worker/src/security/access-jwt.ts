...
function trustedIssuer(teamDomain: string | undefined): URL | null {
  ...
  try {
    const url = teamDomain.startsWith("https://")
      ? new URL(teamDomain)
      : new URL(`https://${teamDomain}`);
    if (
      url.protocol !== "https:" ||
      url.username ||
      url.password ||
...
function validCompactJwt(assertion: string | null): assertion is string {
  ...
  try {
    return assertion
      .split(".")
      .every((segment) => base64url.encode(base64url.decode(segment)) === segment);
  } catch {
    return false;
  }
}
...
export function createAccessVerifier(
  ...
  const cacheSize = Math.max(
    1,
    Math.min(options.maxCachedJwks ?? defaultCacheSize, defaultCacheSize),
  );
  ...
  return {
    async verify(assertion, teamDomain, audience) {
      ...
      if (!issuer || !validAudience(audience) || !validCompactJwt(assertion))
        return null;
      try {
