...
function caseSummary(status = "open") {
  return {
    ...
    petName: "Luna",
    ownerDisplayName: "Familie M.",
    internalReference: "P-12",
    status,
    submissionCount: 1,
    totalBytes: 1234,
...
async function routeAdmin(
  page: Page,
): Promise<{ methods: string[]; urls: string[]; bodies: unknown[] }> {
  const state = {
    methods: [] as string[],
    urls: [] as string[],
    bodies: [] as unknown[],
  ...
  await page.route("**/api/admin/**", async (route) => {
    ...
    if (url.pathname === "/api/admin/session")
      return route.fulfill({
        json: { ok: true, admin: { email: "admin@example.test" } },
      });
    ...
    if (url.pathname === "/api/admin/cases" && request.method() === "POST")
      return route.fulfill({
        ...
        json: {
          ...
          case: caseSummary(),
          token: clearToken,
          shareUrl: `/datentransfer/#token=${clearToken}`,
        },
      });
    if (url.pathname === "/api/admin/cases/case-1" && request.method() === "GET")
      return route.fulfill({ json: caseDetail() });
    if (url.pathname === "/api/admin/cases/case-1/status")
      return route.fulfill({ json: { ok: true, status: "closed" } });
    if (url.pathname === "/api/admin/tokens/token-1/revoke")
      return route.fulfill({ json: { ok: true } });
    if (url.pathname === "/api/admin/cases/case-1/tokens")
      return route.fulfill({
        status: 201,
        json: {
          ok: true,
    ...
    if (url.pathname === "/api/admin/cases/case-1/mark-exported")
      return route.fulfill({ json: { ok: true, exportedAt: "now" } });
...
test("401 bleibt fail-closed und lädt keine Falldaten", async ({ page }) => {
  ...
  await page.route("**/api/admin/**", async (route) => {
    ...
    await route.fulfill({
      status: 401,
      json: { ok: false, error: { requestId: "request-401" } },
    });
  });
  await page.goto("/admin/datentransfer/");

  await expect(
    page.getByRole("heading", { name: "Datentransfer verwalten" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Admin-Zugang erforderlich" }),
  ).toBeVisible();
...
test("Fallanlage zeigt Token einmalig, kopiert Link und verliert ihn beim Reload", async ({
  page,
  context,
}) => {
  ...
  await expect(page.getByLabel("Freigabelink")).toHaveValue(
    `/datentransfer/#token=${clearToken}`,
  );
...
test("Liste, Detail und Verwaltungsaktionen bleiben explizit", async ({ page }) => {
  ...
  await expect(
    page.getByRole("dialog", { name: "Token einmalig sichern" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Schließen", exact: true }).click();
  ...
  expect(
    state.methods.filter((entry) => entry === "POST /api/admin/cases/case-1/tokens"),
  ).toHaveLength(2);
...
test("Adminoberfläche ist bei 320px zugänglich; Druck blendet interne Notiz aus", async ({
  page,
}) => {
  ...
  await expect(page.locator("[data-admin-internal-note-section]")).toHaveCSS(
    "display",
    "none",
  );
});
