...
interface AdminRequestOptions { readonly method?: "GET" | "POST" | "PATCH"; readonly body?: unknown; }
...
export async function requestAdminJson<T = Record<string, unknown>>(
  ...
  if (!path.startsWith("/api/admin/") || path.startsWith("//")) throw new TypeError("Admin-API-Pfad muss same-origin sein.");
  ...
  if (!response.ok || !isRecord(payload) || payload.ok !== true) {
    const base = response.status === 401 ? "Admin-Zugang erforderlich." : "Anfrage fehlgeschlagen.";
    throw new AdminRequestError(`${base}${requestId ? ` Vorgangskennung: ${requestId}` : ""}`, response.status, requestId);
  }
  return payload as T;
}
...
function text(parent: HTMLElement, value: unknown): void { parent.textContent = String(value ?? "—"); }
...
export function setupAdminTransferClient(): void {
  ...
  const setBusy = (control: HTMLButtonElement, busy: boolean): void => { control.disabled = busy; };
  const appendFact = (label: string, value: unknown): void => {
    ...
    text(term, label); text(description, value); wrapper.append(term, description); facts.append(wrapper);
  };
  const showToken = (payload: Record<string, unknown>): void => {
    ...
    text(tokenPetName, transferCase?.petName ?? detailHeading.textContent?.replace(/^Fall für /u, ""));
    text(tokenExpiry, payload.expiresAt ? `Gültig bis ${string(payload.expiresAt)}` : "Token gehört zum neu angelegten Fall.");
    dialog.showModal(); shareUrl.focus(); shareUrl.select();
  };

  const loadCases = async (): Promise<void> => {
    const data = new FormData(filterForm);
    const query = new URLSearchParams({ page: String(page) });
    const selected = string(data.get("status")); const search = string(data.get("q")).trim();
    if (selected) query.set("status", selected); if (search) query.set("q", search);
    const payload = await requestAdminJson(`/api/admin/cases?${query}`);
    const cases = records(payload.cases); total = Number(payload.total) || 0;
    list.replaceChildren();
    if (cases.length === 0) { const empty = document.createElement("p"); text(empty, "Keine Fälle gefunden."); list.append(empty); }
    for (const transferCase of cases) {
      const article = document.createElement("article"); article.className = "admin-case-row";
      const heading = document.createElement("h3"); text(heading, string(transferCase.petName) || "Unbenannter Fall");
      const meta = document.createElement("p"); text(meta, `${string(transferCase.status)} · ${string(transferCase.internalReference) || "ohne interne Referenz"}`);
      const button = document.createElement("button"); button.type = "button"; button.className = "button button--secondary"; text(button, "Details öffnen");
      button.addEventListener("click", () => { void loadDetail(string(transferCase.id)); });
      article.append(heading, meta, button); list.append(article);
    }
    pageLabel.textContent = `Seite ${page}`; previous.disabled = page <= 1; next.disabled = page * 20 >= total;
    setStatus(`${total} Fall/Fälle geladen.`);
  };

  const loadDetail = async (caseId: string): Promise<void> => {
    try {
      const payload = await requestAdminJson(`/api/admin/cases/${encodeURIComponent(caseId)}`);
      if (!isRecord(payload.case)) throw new Error("Falldaten fehlen.");
      const transferCase = payload.case; selectedCaseId = string(transferCase.id); selectedStatus = string(transferCase.status);
      text(detailHeading, `Fall für ${string(transferCase.petName)}`); facts.replaceChildren();
      appendFact("Status", selectedStatus); appendFact("Interne Referenz", transferCase.internalReference);
      appendFact("Anzeigename", transferCase.ownerDisplayName); appendFact("Ablauf", transferCase.expiresAt);
      appendFact("Einsendungen", transferCase.submissionCount); appendFact("Bytes", transferCase.totalBytes);
      const internalNote = string(transferCase.internalNote); noteSection.hidden = !internalNote; text(note, internalNote);
      ...
      for (const token of records(payload.tokens)) {
        ...
        text(label, `Token …${string(token.hint)} · ${token.revokedAt ? "widerrufen" : "aktiv"}`); item.append(label);
        if (!token.revokedAt) { const revoke = document.createElement("button"); revoke.type = "button"; revoke.className = "button button--secondary"; text(revoke, "Token widerrufen"); revoke.addEventListener("click", () => void mutate(revoke, `/api/admin/tokens/${encodeURIComponent(string(token.id))}/revoke`, "POST", {}, true)); item.append(" ", revoke); }
      ...
      const toggle = element<HTMLButtonElement>(detail, "[data-admin-toggle-status]"); text(toggle, selectedStatus === "open" ? "Fall schließen" : "Fall wieder öffnen");
      detail.hidden = false; detailHeading.focus(); setStatus("Falldetail geladen.");
  ...
  createForm.addEventListener("submit", async (event) => {
    event.preventDefault(); const submit = element<HTMLButtonElement>(createForm, "button[type='submit']"); setBusy(submit, true);
    const data = new FormData(createForm);
    try {
      const payload = await requestAdminJson("/api/admin/cases", { method: "POST", body: {
        petName: string(data.get("petName")).trim(), ownerDisplayName: string(data.get("ownerDisplayName")).trim(),
        internalReference: string(data.get("internalReference")).trim(), internalNote: string(data.get("internalNote")).trim(),
        expiresInDays: Number(data.get("expiresInDays")), maxSubmissions: Number(data.get("maxSubmissions")),
        maxTotalBytes: Number(data.get("maxTotalMib")) * 1_024 * 1_024,
        allowReplies: data.has("allowReplies"), allowCallback: data.has("allowCallback"),
      }});
      showToken(payload); createForm.reset(); await loadCases();
    } catch (error) { failClosed(error); } finally { setBusy(submit, false); }
  });
  ...
  void requestAdminJson("/api/admin/session").then((payload) => {
    if (!isRecord(payload.admin)) throw new Error("Admin-Sitzung unvollständig.");
    text(identity, `Angemeldet als ${string(payload.admin.email)}`); authRequired.hidden = true; workspace.hidden = false;
    return loadCases();
  }).catch(failClosed);
}
